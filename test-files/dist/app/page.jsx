...
const AnimatedBackground_1 = __importDefault(require("@/components/AnimatedBackground"));
const firebase_1 = require("@/lib/firebase");
const HamburgerMenu_1 = __importDefault(require("@/components/HamburgerMenu"));
const auth_1 = require("firebase/auth");
const TypingAnimation_1 = __importDefault(require("@/components/TypingAnimation"));
...
function Home() {
    ...
    const [binChats, setBinChats] = (0, react_1.useState)([]);
    const [currentChatId, setCurrentChatId] = (0, react_1.useState)();
    const [isSidebarOpen, setIsSidebarOpen] = (0, react_1.useState)(false);
    const [loginPrompt, setLoginPrompt] = (0, react_1.useState)({ isVisible: false, hasInteracted: false });
    const [aiInteractions, setAiInteractions] = (0, react_1.useState)(0);
    ...
    const loadDataFromLocalStorage = (userId) => {
        ...
        if (savedCollection) {
            try {
                ...
                const collectionItems = parsed.map((item) => ({
                    ...item,
            ...
            if (attachments && attachments.length > 0) {
                ...
                for (const attachment of attachments) {
                    ...
                    try {
                        ...
                        let url = attachment.imageUrl || attachment.videoUrl;
                        if (!content) {
                            if (file.type === 'application/pdf') {
                                console.log('Processing PDF file:', file.name);
                                try {
                                    content = await (0, pdfUtils_1.extractTextFromPDF)(file);
    ...
    return (<AnimatedBackground_1.default>
      {/* Hamburger Menu */}
      <HamburgerMenu_1.default onClick={() => setIsSidebarOpen(!isSidebarOpen)}/>

      {/* Login Prompt */}
      {loginPrompt.isVisible && !firebase_1.auth.currentUser && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-[2px]">
          <div className="bg-[#0a1f15]/90 p-6 rounded-xl shadow-xl max-w-md w-full mx-4 border border-[#00D26A]/20 backdrop-blur-md">
            ...
            <div className="space-y-4">
              <button onClick={() => {
                ...
                (0, auth_1.signInWithPopup)(firebase_1.auth, provider);
      ...
      {/* Toast Notification */}
      {toast && (<div className="fixed top-4 right-4 z-50 flex items-center space-x-2 px-4 py-2 rounded-lg shadow-lg bg-[#1a2e23] text-white">
          {toast.type === 'success' && (<outline_1.CheckCircleIcon className="h-5 w-5 text-[#00ff88]"/>)}
      ...
      <div className="flex h-screen">
        ...
        <div className={`fixed inset-0 lg:relative z-40 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out`}>
          <div className="h-full">
            <Sidebar_1.default collection={collection} onDeleteFromCollection={handleDeleteFromCollection} onNewChat={createNewChat} chatHistory={chatHistory} currentChatId={currentChatId} onSelectChat={(chatId, messages) => {
            handleSelectChat(chatId, messages);
            setIsSidebarOpen(false); // Close sidebar after selection on mobile
        }} onDeleteChat={handleDeleteChat} binChats={binChats} onRestoreChat={handleRestoreChat} onPermanentDelete={handlePermanentDelete} onEmptyBin={handleEmptyBin} onClose={() => setIsSidebarOpen(false)}/>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col w-full">
          ...
          <main className="flex-1 p-6 overflow-y-auto">
            {messages.length === 0 && (<div className="h-full flex flex-col items-center pt-12">
                <div className="mb-12">
                ...
                <div className="hidden md:grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto w-full px-4">
                  ...
                  <div className="p-5 rounded-xl bg-[#052e1f]/40 backdrop-blur-md border border-[#0c2b1c]/30 shadow-lg hover:bg-[#052e1f]/50 transition-colors duration-200">
                    <div className="flex items-center mb-4">
                      <div className="p-1.5 bg-[#00ff88]/10 rounded-lg">
                        <outline_1.ChatBubbleLeftIcon className="h-5 w-5 text-[#00ff88]"/>
                      </div>
                      <h2 className="ml-3 text-lg font-semibold text-white">Examples</h2>
                    </div>
                    <div className="space-y-3">
                      <p onClick={() => handleExampleClick("Explain quantum computing in simple terms")} className="text-[#e2e8f0] text-sm hover:bg-[#0c2b1c]/50 p-3 rounded-lg cursor-pointer transition-colors duration-200">
                        "Explain quantum computing in simple terms" →
                      </p>
                      <p onClick={() => handleExampleClick("How to make ethiopian doro wot easily")} className="text-[#e2e8f0] text-sm hover:bg-[#0c2b1c]/50 p-3 rounded-lg cursor-pointer transition-colors duration-200">
                        "How to make ethiopian doro wot easily" →
                      </p>
                      <p onClick={() => handleExampleClick("How do I make an HTTP request in JavaScript?")} className="text-[#e2e8f0] text-sm hover:bg-[#0c2b1c]/50 p-3 rounded-lg cursor-pointer transition-colors duration-200">
                  ...
                  {/* Capabilities */}
                  <div onClick={() => handleExampleClick("Tell me about your capabilities")} className="p-5 rounded-xl bg-[#052e1f]/40 backdrop-blur-md border border-[#0c2b1c]/30 shadow-lg hover:bg-[#052e1f]/50 transition-colors duration-200 cursor-pointer">
                    <div className="flex items-center mb-4">
                      <div className="p-1.5 bg-[#00ff88]/10 rounded-lg">
                        <outline_1.BoltIcon className="h-5 w-5 text-[#00ff88]"/>
                      </div>
                  ...
                  {/* Limitations */}
                  <div onClick={() => handleExampleClick("What are your limitations?")} className="p-5 rounded-xl bg-[#052e1f]/40 backdrop-blur-md border border-[#0c2b1c]/30 shadow-lg hover:bg-[#052e1f]/50 transition-colors duration-200 cursor-pointer">
                    <div className="flex items-center mb-4">
                      <div className="p-1.5 bg-[#00ff88]/10 rounded-lg">
                        <outline_1.ExclamationTriangleIcon className="h-5 w-5 text-[#00ff88]"/>
                      </div>
            ...
            <div className="space-y-8 max-w-4xl mx-auto">
              {messages.map((message, index) => (<div key={index} className="relative">
                ...
                : 'p-4 sm:p-6 bg-[#0a1f15] mr-auto max-w-[90%] sm:max-w-[800px] rounded-[20px] shadow-lg border border-[#0c2b1c]/30'}>
                    <p className={`text-sm sm:text-base leading-relaxed whitespace-pre-wrap ${message.role === 'user' ? 'text-black' : 'text-white'}`}>{message.content}</p>
                    
                    {/* Display attachments if any */}
                    {message.attachments && message.attachments.length > 0 && (<div className="mt-3 space-y-2">
                        {message.attachments.map((file, fileIndex) => (<div key={fileIndex}>
                            {file.type.startsWith('image/') && file.url ? (<div className="relative group">
                                <img src={file.url} alt={file.name} className="max-h-48 rounded-lg object-contain bg-black/20"/>
                                <a href={file.url} target="_blank" rel="noopener noreferrer" className="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg">
                                  <span className="text-white text-sm">View full size</span>
                                </a>
                              </div>) : (<a href={file.url} target="_blank" rel="noopener noreferrer" className={`flex items-center space-x-2 p-2 rounded-lg hover:opacity-80 transition-opacity ${message.role === 'user'
                            ? 'bg-[#00bf62] text-black'
                            : 'bg-[#1a2e23] text-white'}`}>
                                <outline_1.DocumentTextIcon className="h-5 w-5"/>
                  ...
                  {message.role === 'assistant' && (<div className="absolute -bottom-6 left-4 flex space-x-2">
                      <button onClick={() => handleCopyMessage(message.content)} className="group p-1.5 bg-[#1a2e23] hover:bg-[#243b2f] rounded-lg text-gray-400 hover:text-[#00ff88] transition-all duration-200" title="Copy message">
                        <outline_1.DocumentDuplicateIcon className="h-4 w-4"/>
                      </button>
                      <button onClick={() => handleAddToCollection(message.content)} className="group p-1.5 bg-[#1a2e23] hover:bg-[#243b2f] rounded-lg text-gray-400 hover:text-[#00ff88] transition-all duration-200" title="Add to collection">
                        <outline_1.Square3Stack3DIcon className="h-4 w-4"/>
                      </button>
                    </div>)}
                </div>))}
              {isLoading && (<div className="p-4 sm:p-6 bg-[#0a1f15] mr-auto max-w-[90%] sm:max-w-[800px] rounded-[20px] shadow-lg border border-[#0c2b1c]/30">
                  <TypingAnimation_1.default />
                </div>)}
              {/* Invisible div for scrolling reference */}
              <div ref={messagesEndRef}/>

              {/* New message indicator */}
              {!shouldAutoScroll && messages.length > 0 && (<button onClick={() => {
                ...
                scrollToBottom();
                  ...
                  <outline_1.ArrowDownIcon className="h-4 w-4"/>
                </button>)}
            </div>
          </main>
          ...
          <div className="max-w-4xl mx-auto w-full p-4">
            <ChatInput_1.default onSendMessage={handleSendMessage}/>
          </div>
        </div>
      </div>
    </AnimatedBackground_1.default>);
}
